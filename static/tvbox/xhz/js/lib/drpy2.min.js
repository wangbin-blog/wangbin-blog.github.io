...
function vodDeal(vod) {
  ...
  if (
    (rule.tab_remove && rule.tab_remove.length > 0) ||
    (rule.tab_order && rule.tab_order.length > 0)
  ) {
  ...
  if (!rule.play_parse || !rule.lazy) {
    ...
    try {
      ...
      lazy_play =
        typeof input === "object"
          ? input
          : { parse: 1, jx: tellIsJx(input), url: input };
    } catch (e) {
...
function init(ext) {
  ...
  try {
    ...
    if (rule.url.includes("[") && rule.url.includes("]")) {
      ...
      rule.url =
        rule.host && rule.url ? urljoin(rule.host, rule.url) : rule.url;
    }
    if (
